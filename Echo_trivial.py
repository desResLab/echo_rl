...
class Echo:
    ...
    def _make_reservations(self, fetal_reservation_ratio, nonfetal_reservation_ratio):
        ...
        if num_fetal_room < num_both_sonographer:
            ...
            num_nonfetal_pair = min(num_both_sonographer - num_fetal_room + num_nonfetal_sonographer, num_nonfetal_room)
        else:
            num_fetal_pair = num_both_sonographer
            num_nonfetal_pair = min(num_fetal_room - num_both_sonographer + num_nonfetal_room, num_nonfetal_sonographer)
        ...
        for i in range(num_fetal_pair):
            for room in self.state['rooms']:
                if (room['fetal'] in ['yes','yes (reserved)']) and (room not in reserved_room) and (len(reserved_room) < num_fetal_pair):
                    reserved_room.append(room)
                    self.unreserved_room.remove(room)
                    for sonographer in self.state['sonographers']:
                        if (sonographer['fetal'] in ['yes', 'yes (reserved)']) and (sonographer not in reserved_sonographer) and (sonographer['status'] != 'leave'):
                            reserved_sonographer.append(sonographer)
                            self.unreserved_sonographer.remove(sonographer)
                            self.reserved_fetal_pair.append([room['room number'], sonographer['sonographer number']])
                            break
        for i in range(num_reserved_nonfetal_pair):
            for room in self.state['rooms']:
                if (room not in reserved_room) and (len(reserved_room) < num_fetal_pair + num_reserved_nonfetal_pair):
                    reserved_room.append(room)
                    self.unreserved_room.remove(room)
                    for sonographer in self.state['sonographers']:
                        if (sonographer not in reserved_sonographer) and (sonographer['status'] != 'leave'):
                            reserved_sonographer.append(sonographer)
                            self.unreserved_sonographer.remove(sonographer)
                            self.reserved_nonfetal_pair.append([room['room number'], sonographer['sonographer number']])
                            break
        while len(self.reserved_fetal_pair) > num_reserved_fetal_pair:
            self.unreserved_room.append(self.state['rooms'][self.reserved_fetal_pair[len(self.reserved_fetal_pair) - 1][0]])
            self.unreserved_sonographer.append(self.state['sonographers'][self.reserved_fetal_pair[len(self.reserved_fetal_pair) - 1][1]])
        ...
        if len(reserved_fetal_patients) != 0:
            for i in range(num_reserved_fetal_pair, len(fetal_patients)):
                if fetal_patients[i]['Schedule time'] - reserved_fetal_patients[compared_index]['Schedule time'] >= 40:
    ...
    def _late_waiting_patients(self):
        late_waiting_patients = [patient for patient in self.state['patients']
                          if patient['status'] == 'waiting(late)']
        num = len(late_waiting_patients)
        return num

    def _early_waiting_patients(self):
        early_waiting_patients = [patient for patient in self.state['patients']
                            if patient['status'] in ['waiting(early)']]
        num = len(early_waiting_patients)
        return num

    def _fetal_waiting_patients(self):
        fetal_waiting_patients = [patient for patient in self.state['patients']
                            if patient['fetal'] in ['yes'] and 'waiting' in patient['status']]
        num = len(fetal_waiting_patients)
        return num

    def _nonfetal_waiting_patients(self):
        nonfetal_waiting_patients = [patient for patient in self.state['patients']
                            if patient['fetal'] in ['no'] and 'waiting' in patient['status']]
        num = len(nonfetal_waiting_patients)
        return num
        ...
        if self.env.now >= self.convert_to_step(self.time_close):
            for patient in self.state['patients']:
                if 'waiting' in patient['status']:
                    penalty += 1
        if self.env.now >= self.convert_to_step(self.time_close)+60:
            for patient in self.state['patients']:
                if 'waiting' in patient['status']:
                    penalty += 3
        if self.env.now >= self.convert_to_step(self.time_close)+120:
            for patient in self.state['patients']:
                if 'waiting' in patient['status']:
                    penalty += 5
...
# the function to simulate the steps
    def step(self, action, render):
        ...
        if render:
            self.render()
        return ontime_waiting_patients, late_waiting_patients, early_waiting_patients, fetal_waiting_patients, nonfetal_waiting_patients, terminal
