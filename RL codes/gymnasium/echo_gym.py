...
class Echo(gym.Env):
    def __init__(self, time_start, time_close, num_fetal_room, num_nonfetal_room, num_sonographer_both,
        ...
        self.action_space = spaces.Discrete(729)  # Replace 3 with the number of valid actions

        # Example: Observations are continuous (e.g., room status)
        # Replace with the actual observation space dimensions
        self.observation_space = spaces.Box(
            low=0, high=np.inf, shape=(13,), dtype=np.float32  # Example: 10 continuous features
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        state = [
            ...
            self._onleave_fetal_sonograpphers(),
            self._onleave_nonfetal_sonograpphers(),
            self.env.now
        ]
    ...
    def step(self, action_int):
        ...
        reward = -penalty  # The more penalty, the lower the reward
        ...
        return np.array(state_next, dtype=np.int32), reward, terminated, truncated
    ...
    def _onleave_fetal_sonograpphers(self):
        onleave_fetal_sonograpphers = [sonographer for sonographer in self.state['sonographers']
                                            if sonographer['status'] in ['leave'] and sonographer['fetal'] in ['yes', 'yes (reserved)']]
        num = len(onleave_fetal_sonograpphers)
        return num

    def _onleave_nonfetal_sonograpphers(self):
        onleave_nonfetal_sonograpphers = [sonographer for sonographer in self.state['sonographers']
                                            if sonographer['status'] in ['leave'] and sonographer['fetal'] in ['no']]
        num = len(onleave_nonfetal_sonograpphers)
        return num
...
# Unregister the previous environment if it exists
env_id = 'EchoEnv-v0'
...
register(
    id='EchoEnv-v0',
    entry_point='echo_gym:Echo',  # Replace with your actual module path
    kwargs={
        'time_start': '08:00',
        'time_close': '17:00',
        'num_fetal_room': 1,
