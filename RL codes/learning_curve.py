...
def plot_combined_metrics(penalty_vector, leave_rate_vector, output_path="combined_plot.pdf"):
    ...
    log_penalty = np.log(np.array(penalty_vector))
    smoothed_penalty = (
    ...
    ax1.set_ylabel("Logarithm of Daily Penalty", fontsize=fs, color='b')
    ...
    yticks = np.linspace(0.1, 0.8, 10)  # Creates 10 evenly spaced ticks from 0.1 to 1.0
...
# Load the data
penalty_path = "penalty_vectors/total_penalty_learning_vector_final_ab.npy"
