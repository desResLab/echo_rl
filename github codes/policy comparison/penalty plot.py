...
penalty = [[0] * rows for _ in range(25)]
...
for iteration in range(365):
    for policy in [0,1,2]:
        ...
        if policy == 0:
            state, info = sim1.reset(rate_sonographer_leave=0.1, seed=iteration)  # Reset simulation for each policy
        else:
            sim.reset(rate_sonographer_leave=0.1, seed=iteration)
        if policy == 5:
        ...
        while current_time <= end_time:
            ...
            penalty[policy][current_time] += (-1)*reward/365
            current_time += 1
...
plt.grid(True)
plt.tight_layout()
pdf_path = os.path.join('/Users/bozhisun/Desktop/echo project', 'penalty plot.pdf')
