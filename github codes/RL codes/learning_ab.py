...
class PrioritizedMemory:
    def __init__(self, capacity=MEMORY_SIZE, alpha=0.6, beta=0.4, beta_increment=0.0001):
        self.capacity = capacity
        self.memory = deque(maxlen=capacity)
        self.priorities = deque(maxlen=capacity)
        self.alpha = alpha
        self.beta = beta
        self.beta_increment = beta_increment  # Added beta increment for annealing
        self.epsilon = 1e-6

    def remember(self, state, action, reward, next_state, terminal):
        max_priority = max(self.priorities) if self.priorities else 1.0
        max_priority = min(max_priority, MAX_PRIORITY)  # Apply upper bound
    ...
    def update_priorities(self, indices, errors):
        for idx, error in zip(indices, errors):
            # First add epsilon to avoid zero probabilities
            error_with_epsilon = float(error) + self.epsilon
            # Then clamp between MIN and MAX
            priority = min(max(error_with_epsilon, MIN_PRIORITY), MAX_PRIORITY)
            self.priorities[idx] = priority

    # Added method to update beta over time
...
class DQN(nn.Module):
    def __init__(self):
        ...
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            ...
            patience=100, min_lr=LEARNING_RATE_MIN, verbose=True
        )
    ...
    def act(self, state):
        ...
        for a in action:
            action_int.append(
                a[0] * num_action ** 5 + a[1] * num_action ** 4 + a[2] * num_action ** 3 + a[3] * num_action ** 2 + a[
                    4] * num_action + a[5])
...
def echo_management():
    ...
    env = echo1(time_start=time_start, time_close=time_close, rate_absence=rate_absence, render_env=render_env)
    ...
    for episode in range(1, TRAINING_EPISODES + 1):
        ...
        while not terminated:
            ...
            if len(memory.memory) > (env.convert_to_step(time_close) + 120) * 10:
                if all_steps % EXPLORATION_DECAY_Stride_size == 0:
