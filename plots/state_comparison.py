...
policy_net = DQN(load_path='policy_net_40700.pth')
policy_net.eval()
...
# Initialize storage lists for different actions only
different_states = []
different_actions_1 = []
different_actions_2 = []
...
for i in range(1000,1001):
    ...
    while echo1.env.now <= echo1.convert_to_step(time_close):
        ...
        if action_1 != action_2:
            current_time = (datetime.strptime(time_start, '%H:%M') +
                            timedelta(minutes=int(echo1.env.now))).strftime('%H:%M')
    ...
    ax1.plot(x_positions, different_penalties_1, 'o--', label="Penalty of Trivial policy 2", color='blue')
